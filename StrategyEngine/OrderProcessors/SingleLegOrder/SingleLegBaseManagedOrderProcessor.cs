...
namespace StrategyEngine.OrderProcessors.SingleLegOrder
{
    ...
    {
        internal OrderInfo? ParentOrderReference { get; set; } = null;
        internal OrderInfo? TargetProfitOrderReference { get; set; } = null;
        internal OrderInfo? StopLossLimitOrderReference { get; set; } = null;
    ...
    internal class SingleLegBaseManagedOrderProcessor : AsbtractOrderProcessor<SingleLegBaseManagedOrderProcessor>, IAsyncDisposable, IDisposable
    {
        private bool _disposed = false;
        protected readonly string _tradeFilePath = $"..//..//..//Trades_SingleLegBaseOrderProcessor.csv";
        protected ConcurrentDictionary<string, decimal> _lastTradePrice = [];
        protected TimeSpan _squareOffTime = new TimeSpan(15, 10, 0);
        protected readonly StreamWriter _writer;
        protected ConcurrentDictionary<string, ConcurrentDictionary<Guid, OrderInfo>> _orders = [];

        protected override string Name => $"{GetType().Name}";

        internal SingleLegBaseManagedOrderProcessor(IConfiguration config, Api api, ILoggerFactory loggerFactory)
            : base(config, api, loggerFactory)
        {
            ...
            var header = "StrategyName,OrderCreationTimeStamp,TradingSymbol,Exchange,Token,LimitPrice,Quantity,EntryTime,EntryPrice,TransactionType,StopLossLimitPrice,TargetProfitLimitPrice,ExitTime,ExitPrice,PAndL";
        ...
        {
            _logger.LogInformation("{Processor}: BackgroundWorker task started.", Name);
            while (true)
            {
                    ...
                    foreach (var (symbol, order) in _orders)
                    {
                        foreach (var (orderNumber, orderDetails) in order)
                        {
                                ...
                                orderDetails.TargetProfitOrderReference?.OrderNumber is not null)
                            {
                                ...
                                _orders.GetOrAdd(orderDetails.TradingSymbol, _ => new ConcurrentDictionary<Guid, OrderInfo>())
            ...
            _logger.LogInformation("{Name}: BackgroundAutoSquareOffWorker task finished.", Name);
        }
        ...
        public override Task CreateOrder(string strategyName, CreateOrder createOrder)
        {
            ...
            var stopLossLimitOrder = new OrderInfo
            {
                Exchange = createOrder.Exchange,
                TradingSymbol = createOrder.TradingSymbol,
                Token = createOrder.Token,
            ...
            var targetProfitOrder = new OrderInfo
            {
                Exchange = createOrder.Exchange,
                TradingSymbol = createOrder.TradingSymbol,
                Token = createOrder.Token,
            ...
            if (!_orders.GetOrAdd(parentOrder.TradingSymbol, _ => new ConcurrentDictionary<Guid, OrderInfo>())
                   .TryAdd(parentOrder.OrderNumber, parentOrder))
            {
                _logger.LogWarning("{StrategyName}:[CreateOrder] Ideally this should not be printed.", strategyName);
            }

            _logger.LogInformation("{Processor}: Pending order count in order book [{OrderCount}]", Name, _orders.Sum(c => c.Value.Count));
        ...
        protected override Task OnUpdateInternal(StrategyOnTouchLineSnapshot input)
        {
            ...
            if (_orders.TryGetValue(input.updates.TradingSymbol, out ConcurrentDictionary<Guid, OrderInfo>? orderInfo) && orderInfo is not null)
            {
                ...
                if (orders is not null && orders.Any())
                {
                    foreach (var order in orders)
                    {
                        if (order.ParentOrderReference is null)
                        {
                            ...
                            _orders.GetOrAdd(order.TradingSymbol, _ => new ConcurrentDictionary<Guid, OrderInfo>())
                            ...
                            _logger.LogInformation("{Processor}: Pending order count in order book after parent order execution [{OrderCount}]", Name, _orders.Sum(c => c.Value.Count));
                            continue;
                        }
                        ...
                        _logger.LogInformation("{Processor}: Pending order count in order book after child order execution [{OrderCount}]", Name, _orders.Sum(c => c.Value.Count));
                    }
                }
            }
        ...
        private async ValueTask DisposeAsyncCore()
        {
            ...
            _logger.LogInformation("{Processor}: Disposed gracefully", GetType().Name);
