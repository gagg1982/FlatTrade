...
{
    internal class ExchangeOrderProcessor(IConfiguration config, Api api, ILoggerFactory loggerFactory)
        : AsbtractOrderProcessor<ExchangeOrderProcessor>(config, api, loggerFactory)
    {
        ...
        private decimal GetMarketLimitPriceWithProtection(string tradingSymbol, decimal marketProtection, TransactionType transactionType)
        {
            ...
            var lastTradePriceWithMarketProtection = Utility.RoundToTickWithPrecision((lastTradePrice * marketProtection) / 100, tickSize, pricePrecision);
        ...
        public override async Task CancelOrder(string strategyName, CancelOrder cancelOrder)
        {
            ...
            if (resp is null)
            {
                _logger.LogError("{0}:CancelOrder: Failed cancel order request [{cancelOrder.NorenOrderNumber}]. Error msg {msg}", strategyName, cancelOrder.NorenOrderNumber, msg);
            }
        }
            ...
            switch (modifyOrder.ProductType)
            {
                case ProductType.BracketOrder:
                    switch (modifyOrder.PriceType)
                    {
                        ...
                        default:
                            _logger.LogWarning("{0}:ModifyOrder:{ProductType} Unsupported/unknown PriceType {PriceType} ", strategyName, modifyOrder.ProductType, modifyOrder.PriceType);
                            break;
                    }
                    break;
                ...
                default:
                    _logger.LogWarning("{0}:ModifyOrder: Unsupported/unknown ProductType {ProductType}", strategyName, modifyOrder.ProductType);
                    break;
            }
        }
        ...
        public override async Task CreateOrder(string strategyName, CreateOrder createOrder)
        {
            switch (createOrder.ProductType)
            {
                    ...
                    switch (createOrder.PriceType)
                    {
                        case PriceType.Limit:
                            await CreateOrderBOLimitAsync(createOrder.TradingSymbol,
                                                ...
                                                IsAmo(),
                                                createOrder.InternalOrderId.ToString());
                            break;
                        case PriceType.Market:
                        ...
                        case PriceType.StopLossMarket:
                        default:
                            _logger.LogWarning("{0}:CreateOrder:{ProductType} Unsupported/unknown PriceType {PriceType} ", strategyName, createOrder.ProductType, createOrder.PriceType);
                            break;
                    }
                    break;
                case ProductType.IntraDay:
                ...
                case ProductType.Normal:
                default:
                    _logger.LogWarning("{0}:CreateOrder: Unsupported/unknown ProductType {ProductType}", strategyName, createOrder.ProductType);
                    break;
            }
        }
