...
namespace DailyRunner.FlatTrade
{
    internal class StocksOhlcvGenerator
    {
        ...
        {            
            int chunkSize = 50;
            int cnt = 0;
            var selection = _placeHolderForExchangeTokenSymbolTuple.Select(t => new KeyValuePair<Exchange, long>(t.Item1, t.Item2)).ToList();
            ...
            {
                cnt += exchangeTokenSymbolBatch.Count();
        ...
        private async Task GenerateOhlcvFromQuotes(QuoteSubscriptionRequestAck obj)
        {
            try
            {
                if (!obj.IsValidOhlcv())
                {
                    _logger.LogInformation("GenerateOhlcvFromQuotes: Invalid Ohlcv received for {sym}", obj.TradingSymbol);
                    return;
                }
                ...
                var headerAndContents = GenerateHeaderAndContents([obj]);
                if (_csvWriter_1440 is not null)
                {
                    string newFile = $"CurrentDayOHLCV_{obj.TradingSymbol}_{(int)ChartInterval.Daily}_{DateTime.Now:yyyyMMdd}.csv";
            ...
            {
                _logger.LogError(ex, "An error occurred while generating the Ohlcv CSV from quotes: {ex.Message}", ex.Message);
            }
        }
        ...
        private async Task OnQuoteUpdates(object? _, SubscriptionType subscriptionType, string rawMessage, object? subscriptionObject)
        {
            ...
            switch (subscriptionType)
            {
                ...
                case SubscriptionType.UnsubscribeQuoteAck:
                    _logger.LogDebug("[OnQuoteUpdates-UnsubscribeQuoteAck] {msg}", msg);                    
                    break;
                                                          ...
                                                          .Select(val => (val.Token, new PriceCandle
                                                             {
                                                                 ...
                                                                 High = Math.Max(val.DayHighPrice, val.LastTradePrice),
                                                                 Low = Math.Min(val.DayLowPrice, val.LastTradePrice),
                                                                 Close = val.LastTradePrice,
