...
namespace Common.Helpers
{
    public static class RestHttpClientExtension
    {
        ...
        public static async Task<(TReceive?, string)> GetMessageAsync<TReceive, TError>(this RestHttpClient restHttpClient, string uri, bool isSchemaModelCheckByPassed = false) where TError : BaseErrorMessage
        {
            var (responseString, status) = await GetMessageHelperAsync<TError>(restHttpClient, uri); // Pass 'restHttpClient' explicitly to fix CS7036
            ...
            {
                eMsg = $"[API Client Error]: GET JSON Reader Exception for {uri}: {e.Message} - Raw Response: '{responseString}'";
            }
            catch (JsonSerializationException e)
            {
                eMsg = $"[API Client Error]: GET JSON Serialization Exception for {uri}: {e.Message} - Raw Response: '{responseString}'";
            }
            catch (ArgumentOutOfRangeException e)
            {
                eMsg = $"[API Client Error]: GET JSON ArgumentOutOfRange Exception for {uri}: {e.Message} - Raw Response: '{responseString}'";
            }
            catch (Exception e)
            {
                eMsg = $"[API Client Error]: Unexpected error during GET for {uri}: {e.Message} - Raw Response: '{responseString}'";
            }
            return (default, eMsg);
        }

        public static async Task<(TReceive?, string)> PostMessageAsync<TReceive, TError>(this RestHttpClient restHttpClient, string uri, string msg, bool isSchemaModelCheckByPassed = false) where TError: BaseErrorMessage
        {
            var (responseString, status) = await PostMessageHelperAsync<TError>(restHttpClient, uri, msg); // Pass 'restHttpClient' explicitly to fix CS7036
            if (string.Compare(status, Constants.StatusOk, StringComparison.InvariantCultureIgnoreCase) != 0)
            {
                return (default, $"{status} {responseString}");
            }
        ...
        private static string? GetErrorResponseIfAny<TError>(string responseString) where TError : BaseErrorMessage
        {
            ...
            try
            {
                ...
                {
                    var anyErrorResponse = JsonConvert.DeserializeObject<IEnumerable<TError>>(responseString);
                ...
                {
                    errorResponse = JsonConvert.DeserializeObject<TError>(responseString);
                }
        ...
        private static async Task<(string, string)> PostMessageHelperAsync<TError>(RestHttpClient restHttpClient, string uri, string msg) where TError : BaseErrorMessage
        {
            HttpContent content = new StringContent(msg, Encoding.UTF8);
            string? errorMsg;
            try
            {
                ...
                errorMsg = GetErrorResponseIfAny<TError>(responseString);
        ...
        private static async Task<(string, string)> GetMessageHelperAsync<TError>(RestHttpClient restHttpClient, string uri) where TError : BaseErrorMessage
        {
            string? errorMsg;
            try
            {
                var (responseString, eMsg) = await restHttpClient.GetAsync(new Uri(uri));
                if (string.Compare(eMsg, Constants.StatusOk, StringComparison.InvariantCultureIgnoreCase) != 0 &&
                   string.IsNullOrEmpty(responseString))
                {
                    return (string.Empty, eMsg);
            ...
            {
                errorMsg = $"[API Client Error]: GET JSON Serialization Exception for {uri}: {e.Message}";
            }
            catch (ArgumentOutOfRangeException e)
            {
                errorMsg = $"[API Client Error]: GET JSON Serialization Exception for {uri}: {e.Message}";
            ...
            {
                errorMsg = $"[API Client Error]: Unexpected error during GET for {uri}: {e.Message}";
            }
